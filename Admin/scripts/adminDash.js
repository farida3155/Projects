 ...
 function renderPets(pets) {
   ...
   // Add card flip on click (not on button)
 ...
 function petCardHTML(pet) {
   // Default image path
   const defaultImg = '/images/pawdefault.png';
 ...
 function showPetFormModal(mode, pet = {}) {
   ...
   document.getElementById('petCardContainer').innerHTML = `
     <div class="flip-card" style="pointer-events:auto;">
       <div class="flip-card-inner" style="transform:none;">
         <div class="flip-card-front" style="padding-top:18px;">
           <form id="petForm" enctype="multipart/form-data" style="width:100%;">
             ${isUpdate ? `<input type="hidden" name="petId" value="${pet._id}">` : ""}
             <img id="previewImg" src="${imgUrl}" alt="Pet Image" style="margin:0 auto 18px auto; display:block;">
             <input type="file" id="imageInput" name="image" accept="image/*" style="margin-bottom:12px;">
             <div class="petinfo" style="margin-bottom:7px;">
               <input type="text" name="name" id="petName" placeholder="Name" required
                 style="width:82%;font-size:1.09rem;text-align:center;" value="${pet.name || ''}">
             </div>
             <div class="petinfo" style="margin-bottom:7px;">
               <input type="number" min="0" name="ageValue" id="petAgeValue" placeholder="Age" required
                 style="width:48px;" value="${pet.age ? pet.age.value : ''}">
               <select name="ageUnit" id="petAgeUnit" required style="width:70px;">
             ...
             <div class="petinfo" style="margin-bottom:7px;">
               <input type="text" name="breed" id="petBreed" placeholder="Breed" required
                 style="width:62%;" value="${pet.breed || ''}">
             </div>
             <div class="petinfo" style="margin-bottom:7px;">
               <select name="gender" id="petGender" required style="width:70%;">
                 <option value="">Gender</option>
                 <option value="male" ${pet.gender === "male" ? "selected" : ""}>Male</option>
                 <option value="female" ${pet.gender === "female" ? "selected" : ""}>Female</option>
               </select>
               <select name="neutered" id="petNeutered" required style="width:60%;">
             ...
             <div class="petinfo" style="margin-bottom:7px;">
               <input type="text" name="location" id="petLocation" placeholder="Location" required
                 style="width:80%;" value="${pet.location || ''}">
             </div>
             <div class="petinfo" style="margin-bottom:7px;">
               <select name="type" id="petType" required style="width:80%;">
             ...
             <div class="petinfo" style="margin-bottom:7px;">
               <label class="toggle-switch">
                 <input type="checkbox" id="petUpForAdoption" name="upForAdoption" ${pet.upForAdoption !== false ? "checked" : ""}>
                 <span>${pet.upForAdoption === false ? "Not For Adoption" : "Up For Adoption"}</span>
               </label>
             </div>
             <button type="submit" class="managepets-btn" style="margin-top:10px;">
   ...
   // Toggle label update
   const upForAdoptionBox = document.getElementById('petUpForAdoption');
 ...
 function showRemoveConfirmModal(pet) {
   ...
   document.getElementById('petModal').style.display = 'none'; // Close pet modal if open
